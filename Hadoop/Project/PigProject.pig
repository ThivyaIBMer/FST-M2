-- Load data from HDFS
inputDialogues4 = LOAD 'hdfs:///user/thivya/inputs/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/thivya/inputs/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/thivya/inputs/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
...
rmf hdfs:///user/thivya/output;

-- Store result in HDFS
STORE namesOrdered INTO 'hdfs:///user/thivya/output' USING PigStorage('\t');
